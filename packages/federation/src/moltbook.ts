...
/**
 * Human-readable threat report generated by analyzing agents.
 */
export interface ThreatReport {
  /** Summary of the threat (1-2 sentences) */
  readonly summary: string;
  
  /** Detailed analysis explaining the reasoning */
  readonly analysis: string;
  
  /** Specific evidence cited with explanations */
  readonly evidenceCited: string[];
...
/**
 * Generate human-readable threat reports for case review.
 */
export function generateThreatReport(
  signal: MoltbookThreatSignal,
  matchedText: string,
  semanticResult?: SemanticAnalysisResult
): ThreatReport {
  const threatDescriptions: Record<MoltbookThreatType, { name: string; danger: string }> = {
    credential_theft: { name: 'Credential Theft', danger: 'stealing user authentication credentials or API keys' },
    scam: { name: 'Scam Activity', danger: 'defrauding users through fake promotions or schemes' },
    spam: { name: 'Spam', danger: 'flooding the platform with promotional or irrelevant content' },
    manipulation: { name: 'Social Manipulation', danger: 'psychologically manipulating users for malicious purposes' },
    malware_distribution: { name: 'Malware Distribution', danger: 'distributing malicious code or harmful software' },
    impersonation: { name: 'Impersonation', danger: 'pretending to be another agent or human to deceive users' },
    data_harvesting: { name: 'Data Harvesting', danger: 'collecting personal information without consent' },
    coordinated_attack: { name: 'Coordinated Attack', danger: 'working with other agents to conduct malicious activity' },
    prompt_injection: { name: 'Prompt Injection', danger: 'attempting to manipulate other AI agents through malicious prompts' },
    typosquatting: { name: 'Typosquatting', danger: 'using deceptive names similar to legitimate services' },
    financial_fraud: { name: 'Financial Fraud', danger: 'conducting fraudulent financial schemes' },
    phishing: { name: 'Phishing', danger: 'using fake links to steal credentials or data' },
    harassment: { name: 'Harassment', danger: 'targeting and harassing specific users' },
    disinformation: { name: 'Disinformation', danger: 'deliberately spreading false information' },
    resource_abuse: { name: 'Resource Abuse', danger: 'misusing platform resources at scale' },
  };

  const threatInfo = threatDescriptions[signal.type];
  const severityDescriptions: Record<string, string> = {
    critical: 'poses an immediate and severe threat to users',
    high: 'represents a significant risk that warrants prompt attention',
    medium: 'indicates concerning behavior that should be monitored',
    low: 'suggests minor policy violations or suspicious patterns',
  };

  // Build summary
  const summary = `Agent "${signal.agentUsername}" flagged for ${threatInfo.name} (${signal.severity} severity). ` +
    `This activity involves ${threatInfo.danger}.`;

  // Build detailed analysis
  const analysisLines: string[] = [
    `**Detection Type:** ${threatInfo.name}`,
    `**Severity Level:** ${signal.severity.toUpperCase()} - This ${severityDescriptions[signal.severity]}.`,
    `**Confidence Score:** ${(signal.confidence * 100).toFixed(0)}%`,
    '',
    `**What Was Detected:**`,
    `The agent's content contained: "${matchedText.slice(0, 200)}${matchedText.length > 200 ? '...' : ''}"`,
    '',
    `**Why This Is Suspicious:**`,
  ];

  // Add specific reasoning based on threat type
  switch (signal.type) {
    case 'data_harvesting':
      analysisLines.push(
        `The phrase requests users share sensitive personal information (wallet addresses, emails, phone numbers, etc.).`,
        `This is a common tactic used in data harvesting attacks where malicious actors collect PII for`,
        `identity theft, targeted scams, or selling on dark markets.`
      );
      break;
    case 'credential_theft':
      analysisLines.push(
        `The content requests API keys, passwords, tokens, or other authentication credentials.`,
        `No legitimate service should ever ask users to share their credentials through social posts.`,
        `This is indicative of credential harvesting for account takeover attacks.`
      );
      break;
    case 'scam':
      analysisLines.push(
        `The content promotes get-rich-quick schemes, fake giveaways, or suspicious investment opportunities.`,
        `These patterns are commonly associated with cryptocurrency scams and pump-and-dump schemes`,
        `designed to defraud unsuspecting users.`
      );
      break;
    case 'phishing':
      analysisLines.push(
        `The content contains suspicious links that may impersonate legitimate services.`,
        `URL shorteners or domains similar to (but not) official services are common phishing tactics`,
        `used to steal login credentials when users click through.`
      );
      break;
    case 'manipulation':
      analysisLines.push(
        `The content uses psychological manipulation techniques such as urgency, fear, or authority appeals.`,
        `These tactics are designed to bypass critical thinking and pressure users into actions`,
        `they would not normally take.`
      );
      break;
    case 'prompt_injection':
      analysisLines.push(
        `The content contains patterns that could manipulate other AI agents' behavior.`,
        `Prompt injection attacks attempt to override an AI's instructions, potentially causing`,
        `the target agent to execute unintended or malicious actions.`
      );
      break;
    default:
      analysisLines.push(
        `The content matches known patterns associated with ${threatInfo.name.toLowerCase()}.`,
        `This type of activity ${threatInfo.danger}.`
      );
  }

  // Add semantic analysis if available
  if (semanticResult && semanticResult.overallRisk > 0.4) {
    analysisLines.push('', '**Semantic Analysis:**');
    if (semanticResult.manipulation.score > 0.4) {
      analysisLines.push(`- Manipulation signals detected (${(semanticResult.manipulation.score * 100).toFixed(0)}%): ${semanticResult.manipulation.indicators.join(', ')}`);
    }
    if (semanticResult.deception.score > 0.4) {
      analysisLines.push(`- Deception indicators (${(semanticResult.deception.score * 100).toFixed(0)}%): ${semanticResult.deception.indicators.join(', ')}`);
    }
    if (semanticResult.urgency.score > 0.4) {
      analysisLines.push(`- High-pressure tactics (${(semanticResult.urgency.score * 100).toFixed(0)}%): ${semanticResult.urgency.indicators.join(', ')}`);
    }
  }
  ...
  const evidenceCited = signal.evidence.map((e, i) => {
    const evidenceType = e.type === 'pattern' ? 'Pattern Match' : e.type === 'behavior' ? 'Behavioral Signal' : 'Evidence';
    return `${i + 1}. **${evidenceType}**: ${typeof e.data === 'string' ? `"${e.data}"` : 'Complex data structure'} (${(e.confidence * 100).toFixed(0)}% confidence)`;
  });

  // Risk assessment
  const riskAssessment = `Based on a ${(signal.confidence * 100).toFixed(0)}% confidence detection with ${signal.evidence.length} supporting evidence item(s), ` +
    `this case is classified as **${signal.severity.toUpperCase()}** severity. ` +
    (signal.severity === 'critical' || signal.severity === 'high'
      ? `Immediate review is recommended due to the potential for significant user harm.`
      : `This case should be monitored and reviewed in the normal queue.`);

  // Recommendation
  const recommendationByTier: Record<string, string> = {
    critical: 'Recommend immediate escalation to Tier 4 (Isolation). The agent should be suspended pending full investigation.',
    high: 'Recommend Tier 3 action (Shadow-ban). Limit agent visibility while conducting deeper forensic analysis.',
    medium: 'Recommend Tier 2 action (Voluntary throttling). Rate-limit the agent and monitor for continued violations.',
    low: 'Recommend Tier 1 action (Advisory). Issue a warning and add to watchlist for future monitoring.',
  };

  const recommendation = recommendationByTier[signal.severity];

  return { summary, analysis, evidenceCited, riskAssessment, recommendation };
}
